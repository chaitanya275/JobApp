  ...
  navigate("/edit",{state:{id}});
    ...
    useEffect(() => {
      const fetchPosts = async () => {
        const response = await axios.get(`http://localhost:8090/jobPosts/keyword/${query}`);    
        setPost(response.data);
      };
        const fetchInitialPosts = async () => {
            const response = await axios.get(`http://localhost:8090/jobPosts`);
            setPost(response.data);
        }
         fetchInitialPosts();
      ...
      const handleDelete = (id) => {
        async function deletePost() {
          await axios.delete(`http://localhost:8090/jobPost/${id}`);
      }
      deletePost();
